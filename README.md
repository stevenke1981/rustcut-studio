...
- 字幕與視覺特效：標準、Hormozi、極簡、逐字卡拉 OK、逐字彈出字幕，以及靜態／動態字卡、文字底框與陰影、全片影片邊框及浮水印。
...
- 瀏覽器工作區：素材、播放器、時間軸、字幕與特效工具箱、AI 對話三欄介面。
...
開啟 `http://127.0.0.1:8787`。左側「字幕與特效」工具箱可直接加入字幕、
字卡、動態字卡、影片邊框與全片浮水印；所有操作都可使用上方的復原／重做。
字幕可選擇五種預設；卡拉 OK 與逐字彈出會直接使用匯入逐字稿的 word timestamps。
...
- 瀏覽器介面是可操作的工程版 UI，不是完整 NLE；字卡提供預設動畫，但尚未包含滑鼠拖曳、波形、影格縮圖與任意 keyframe 編輯器。
